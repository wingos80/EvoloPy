...
import optimizers.CMAES as cmaes
import benchmarks
import csv
import numpy
import time
...
import plot_times as times_plot
import plot_spreads as spreads_plot
...
def selector(algo, func_details, popSize, Iter):
    ...
    if algo == "SSA":
        ...
        x = cmaes.CMAES(getattr(benchmarks, function_name), lb, ub, dim, popSize, Iter)
    else:
        return null
    return x
...
def run(optimizer, objectivefunc, NumOfRuns, params, export_flags):
    ...
    print("\nFinished optimizations, beginning plotting\n")
    if Export_convergence == True:
        conv_plot.run(results_directory, optimizer, objectivefunc, Iterations)
        spreads_plot.run(results_directory, optimizer, objectivefunc, Iterations)

    if Export_boxplot == True:
        box_plot.run(results_directory, optimizer, objectivefunc, Iterations)
        times_plot.run(results_directory, optimizer, objectivefunc, Iterations)
